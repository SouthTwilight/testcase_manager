...
class TestCaseExecutor:
    def __init__(self, max_workers=None):
        self.max_workers = max_workers or Config.MAX_CONCURRENT_TESTS
    ...
    def execute_single_case(self, test_case, plan_id=None):
        ...
        try:
            ...
            if test_case.full_path.endswith('.py'):
                # 执行Python测试用例
                cmd = ['python', '-m', 'pytest', test_case.full_path, '-v', '--tb=short']
    ...
    def execute_test_plan(self, plan_id):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for case in cases:
                # 提交任务
                future = executor.submit(self.execute_single_case, case, plan.id)
            ...
            for future in as_completed(future_to_case):
                ...
                try:
                    status, duration, details = future.result()
                    ...
                    print(f"Failed to execute case {case.case_hash}: {str(e)}")
